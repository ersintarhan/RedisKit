...
public abstract class IntegrationTestBase : IAsyncLifetime
{
    ...
    public virtual async Task InitializeAsync()
    {
        ...
        try
        {
            ...
            {
                Console.WriteLine("WARNING: IRedisFunction service not registered");
            }
            if (ShardedPubSubService == null)
            {
                Console.WriteLine("WARNING: IRedisShardedPubSub service not registered");
            }
        }
        catch (Exception ex)
        {
        ...
        {
            throw new InvalidOperationException($"Redis connection is too slow: {ping.TotalMilliseconds}ms");
        }
    ...
    public virtual async Task DisposeAsync()
    {
        try
        {
            ...
            // Clean up streams
            foreach (var stream in _streamsToClean)
            {
    ...
    {
        if (!await IsRedis7Available())
        {
            throw new SkipException("Redis 7.0+ required for this test");
        }
    }
}
