...
internal class SystemTextJsonRedisSerializer : IRedisSerializer
{
    ...
    public byte[] Serialize<T>(T obj)
    {
        try
        {
            ...
            var result = JsonSerializer.SerializeToUtf8Bytes(obj, Options);
    ...
    public Task<byte[]> SerializeAsync<T>(T obj, CancellationToken cancellationToken = default)
    {
        ...
        if (_logger?.IsEnabled(LogLevel.Warning) == true) ValidateObjectSize(obj);
    ...
    private void ValidateObjectSize<T>(T obj)
    {
        ...
        switch (obj)
        {
            ...
            case ICollection collection when collection.Count > maxCollectionItems:
                _logger?.LogLargeObjectWarning("Collection", collection.Count);
                break;

            case IDictionary dict when dict.Count > maxCollectionItems:
