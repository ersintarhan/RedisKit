...
internal static class StreamSerializationHelper
{
    ...
    public static NameValueEntry[] CreateDataFields<T>(T[] messages, IRedisSerializer serializer)
    {
        ...
        for (var i = 0; i < messages.Length; i++) fields[i] = CreateDataField(messages[i], serializer);
        return fields;
    }
    ...
    public static T? DeserializeStreamEntry<T>(StreamEntry entry, IRedisSerializer serializer) where T : class
    {
        ...
        if (dataField.HasValue && !dataField.IsNullOrEmpty) return serializer.Deserialize<T>(dataField!);
        return default;
    }
        ...
        IRedisSerializer serializer) where T : class
    {
        ...
        foreach (var entry in entries)
        {
            ...
            if (data != null) results.Add((entry.Id.ToString(), data));
        }
        ...
        int batchSize = 100)
    {
        ...
        for (var i = 0; i < messages.Length; i += batchSize)
        {
            ...
            for (var j = 0; j < batchResults.Length; j++) results[i + j] = new[] { batchResults[j] };
        }
